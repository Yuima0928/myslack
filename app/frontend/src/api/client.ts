...
export const api = {
  ...
  async joinSelf(wsId: string, channelId: string) {
    return authedJson<{ ok: boolean }>(`/workspaces/${wsId}/channels/${channelId}/join`, {
      method: 'POST',
    });
  },
  ...
  // アバター用の署名URL発行
  async signUploadAvatar(input: { filename: string; content_type: string; size_bytes: number }) {
